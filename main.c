...
void testStrongEncryption() {
    acp_init();
    char* encrypted = crypt("Hello", "d3bkvc5z", NULL, md5StringHash, 1, 0, 0);
    printf("%s\n", encrypted);
    char* decrypted = crypt(encrypted, "d3bkvc5z", NULL, md5StringHash, 0, 0, 0);
    printf("%s\n", decrypted);
}
...
void testStrongEncryptionBinary() {
    ...
    data[4] = 'o';
    char* result;
    char* encrypted = bcrypt(data, 5, "d3bkvc5z", NULL, md5StringHash, 1, 0, 0);
    for (int i = 0; i < 6; i++) {
    ...
    printf("\n");
    char* decrypted = bcrypt(encrypted, 5 + 1 + sizeof(int) * 2, "d3bkvc5z", NULL, md5StringHash, 0, 0, 0);
    printf("%s\n", decrypted);
}
...
void testBinaryFileEncryption() {
    bcrypt_file("pic.png", "pic.png.acp", "d3bkvc5z", md5StringHash, 1, 0, 0);
    bcrypt_file("pic.png.acp", "pic2.png", "d3bkvc5z", md5StringHash, 0, 0, 0);
